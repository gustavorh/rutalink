...
export function DashboardSidebar({
  ...
  return (
    <aside
      ...
      } flex-shrink-0 bg-ui-sidebar-bg border-r border-border shadow-xl p-6 flex flex-col transition-all duration-300 relative z-20`}
    >
      {/* Toggle Button */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-8 bg-ui-surface-elevated hover:bg-ui-surface-hover text-foreground rounded-full p-1.5 shadow-xl z-30 transition-all hover:scale-110 border border-border ring-2 ring-ui-bg"
      ...
      {/* Brand */}
      <div className="mb-8 flex items-center gap-2 pb-6 border-b border-border/50">
        <Image
          src="/logo-bilix.png"
      ...
      <nav className={`flex-1 ${isCollapsed ? "space-y-4" : "space-y-1"}`}>
          ...
          <button
            ...
            className={`w-full flex items-center ${
              ...
              isActive("/dashboard/reports")
                ...
                : "text-foreground hover:bg-ui-surface-elevated hover:text-foreground hover:shadow-sm"
            }`}
        ...
        {/* Divider */}
        <div className={`${isCollapsed ? "my-4" : "my-6"}`}>
          {!isCollapsed ? (
            <div className="border-t border-border"></div>
          ) : (
            <div className="h-px bg-border mx-auto w-8"></div>
          )}
        </div>
          ...
          {!isCollapsed && isAdministrationOpen && (
            <div className="ml-4 mt-1 space-y-1">
              <div
                ...
                className={`px-3 py-2 text-xs cursor-pointer rounded transition-colors ${
                  isActive("/administration/operators")
                    ? "bg-primary/20 text-primary border-l-2 border-primary font-medium"
                    : "text-muted-foreground hover:text-foreground hover:bg-ui-surface-hover"
                }`}
              >
              ...
              <div
                onClick={() => onNavigate("/administration/roles")}
                className={`px-3 py-2 text-xs cursor-pointer rounded transition-colors ${
              ...
              <div
                onClick={() => onNavigate("/administration/users")}
                className={`px-3 py-2 text-xs cursor-pointer rounded transition-colors ${
